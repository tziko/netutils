...
public class EthernetFrame
{
	...
	public int getPacketType()
	{
		...
		{
			myType = ByteUtils.getByteNetOrderTo_unit16(myPacket,ETHERNET_TYPE_OFFSET);
			// vlan
			if(myType == 0x8100)
			{
				myType = ByteUtils.getByteNetOrderTo_unit16(myPacket,ETHERNET_TYPE_OFFSET+4);
				myOffset+=4;
			}
			_isReadType = true;
		}
	...
	{
		int type =  ByteUtils.getByteNetOrderTo_unit16(thePacket,ETHERNET_TYPE_OFFSET);
		
		// if vlan then get correct offset
		if (( type & 0xff00) == ETHERNET_OVER_VLAN)
		{
			return ByteUtils.getByteNetOrderTo_unit16(thePacket,ETHERNET_TYPE_OFFSET+4);
		}
		return type;
	}
